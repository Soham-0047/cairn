...
const Landing = ({ navigate }) => {
  const [demoOpen, setDemoOpen] = lUseState(false);
  return (
    <div className="page-enter">
      <Hero navigate={navigate} onDemo={()=>setDemoOpen(true)} />
      <Marquee />
      <ProblemScene />
      <HowItWorks />
      <BentoEvaluation />
      ...
      <Modal open={demoOpen} onClose={()=>setDemoOpen(false)} width={840}>
        <div style={{ aspectRatio:'16/9', borderRadius:14, background:'linear-gradient(135deg, #1a1d2e, #0e1019)', position:'relative', overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center'}}>
          <AuroraBackground intensity={.6}/>
          <div style={{ position:'relative', zIndex:2, textAlign:'center'}}>
            <Icon name="play" size={56} style={{color:'#fff', opacity:.9, filter:'drop-shadow(0 0 20px rgba(99,102,241,0.6))'}}/>
            <div className="serif italic" style={{ fontSize: 28, marginTop: 14 }}>The 90-second tour</div>
            <div style={{ color:'var(--text-mid)', marginTop:6, fontSize:14 }}>Goal → path → projects → credentials → portfolio</div>
          </div>
        </div>
      </Modal>
    </div>
...
/* ---------------- HERO ---------------- */
const Hero = ({ navigate, onDemo }) => {
  return (
    <section id="hero" className="grain" style={{ minHeight:'100vh', position:'relative', overflow:'hidden', display:'flex', alignItems:'center' }}>
      <AuroraBackground/>
      {/* Grid underlay */}
      <svg width="100%" height="100%" style={{position:'absolute', inset:0, opacity:.07}} aria-hidden="true">
        <defs>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1.2fr 1fr', gap: 60, alignItems:'center' }} className="hero-grid">
          <div>
            ...
            <h1 className="serif" style={{
              ...
              lineHeight: 0.96, margin:0, letterSpacing:'-0.035em', textWrap:'balance'
            }}>
              <SplitText>Turn the chaos of free tutorials into a</SplitText>{' '}
              <span className="italic grad-text" style={{ position:'relative', animation:'fadeUp .8s .4s cubic-bezier(.16,1,.3,1) both', display:'inline-block' }}>
                verified path
              </span>{' '}
              <SplitText delay={900}>to your next career.</SplitText>
            </h1>
            <p style={{
              marginTop: 28, fontSize: 20, color:'var(--text-mid)', maxWidth: 560, lineHeight:1.5,
            ...
            <div style={{ display:'flex', gap:12, marginTop: 36, animation:'fadeUp .8s 1.6s cubic-bezier(.16,1,.3,1) both' }}>
              <MagneticButton onClick={()=>navigate('onboarding')}>
            ...
            <div style={{ display:'flex', gap:24, marginTop: 36, color:'var(--text-mid)', fontSize:13, animation:'fadeUp .8s 1.8s cubic-bezier(.16,1,.3,1) both' }}>
              <span style={{display:'inline-flex', alignItems:'center', gap:6}}><Icon name="check" size={14} style={{color:'#6ee7b7'}}/> No credit card</span>
              <span style={{display:'inline-flex', alignItems:'center', gap:6}}><Icon name="check" size={14} style={{color:'#6ee7b7'}}/> Free Gemma routing</span>
              <span style={{display:'inline-flex', alignItems:'center', gap:6}}><Icon name="check" size={14} style={{color:'#6ee7b7'}}/> Recruiter-shareable</span>
            </div>
          </div>
          <div style={{ display:'flex', justifyContent:'center', alignItems:'center', position:'relative' }} className="hero-right">
            <div style={{ position:'absolute', inset:'-10%', background:'radial-gradient(circle, var(--primary-glow), transparent 60%)', filter:'blur(40px)', zIndex:0 }}/>
            <CairnStack/>
          </div>
        </div>
      </div>
      {/* Scroll indicator */}
      <div style={{ position:'absolute', bottom:32, left:'50%', transform:'translateX(-50%)', display:'flex', flexDirection:'column', alignItems:'center', gap:6, color:'var(--text-mid)', fontSize:11, letterSpacing:'.14em', animation:'fadeUp 1s 2.2s both' }}>
        <span>SCROLL</span>
        <svg width="16" height="22"><rect x="0.5" y="0.5" width="15" height="21" rx="7" fill="none" stroke="currentColor" opacity=".5"/><circle cx="8" cy="7" r="2" fill="currentColor"><animate attributeName="cy" values="7;15;7" dur="2s" repeatCount="indefinite"/></circle></svg>
      </div>
      <style>{`
...
const Marquee = () => {
  ...
  return (
    <section style={{ padding:'40px 0', borderTop:'1px solid var(--border)', borderBottom:'1px solid var(--border)', overflow:'hidden', background:'var(--bg-0)'}}>
      <div className="container" style={{ display:'flex', alignItems:'center', gap:40 }}>
        <div style={{ fontSize:12, color:'var(--text-mid)', textTransform:'uppercase', letterSpacing:'.16em', flexShrink:0 }}>Trusted by learners from</div>
        <div style={{ flex:1, overflow:'hidden', position:'relative' }}>
          <div className="marquee-track">
            {list.map((l,i) => (
              <div key={i} className="serif italic" style={{ fontSize:28, color:'var(--text-mid)', opacity:.7 }}>{l}</div>
            ))}
          </div>
    ...
    <section ref={ref} style={{ height:'250vh', position:'relative' }}>
      <div style={{ position:'sticky', top:0, height:'100vh', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
        ...
        <div className="container" style={{ textAlign:'center', position:'relative', zIndex:2 }}>
          <div className="pill pill-warm" style={{marginBottom:24}}>The problem with self-directed learning</div>
          <div style={{ position:'relative', height: 200 }}>
            {phrases.map((p, i) => {
              const start = i*seg, end = (i+1)*seg;
              const local = Math.max(0, Math.min(1, (progress - start) / seg));
              const opacity = i === phrases.length - 1
                ? (progress >= start ? Math.min(1, local*2) : 0)
                : Math.min(1, Math.sin(local * Math.PI) * 1.5);
              const y = (1 - local) * 20;
              return (
                <div key={i} style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', opacity, transform:`translateY(${y}px)`, transition:'all .3s ease' }}>
                  <h2 className="serif italic" style={{ fontSize:'clamp(40px, 7vw, 96px)', lineHeight:1, margin:0, letterSpacing:'-.03em' }}>{p.t}</h2>
                  <div style={{ marginTop: 24, fontFamily:'var(--mono)', fontSize:13, color:'var(--text-mid)', letterSpacing:'.16em', textTransform:'uppercase'}}>{p.n} {p.label}</div>
                </div>
              );
            })}
          </div>
...
/* ---------------- How it works (4 steps horizontal feel) ---------------- */
const HowItWorks = () => {
  const steps = [
  ...
  return (
    <section id="how" style={{ padding:'160px 0', position:'relative' }}>
      <div className="container">
        ...
        <div style={{ marginTop: 64, display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap: 24 }} className="how-grid">
          {steps.map(s => (
            <div key={s.n} className="card" style={{ padding: 24, position:'relative', overflow:'hidden', minHeight: 360 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom: 20 }}>
                <div className="mono" style={{ fontSize:11, padding:'4px 8px', borderRadius:6, background:'var(--bg-2)', boxShadow:'inset 0 0 0 1px var(--border-strong)', color:'var(--text-mid)' }}>STEP {s.n.toString().padStart(2,'0')}</div>
              </div>
              <div style={{ height:140, marginBottom: 20, position:'relative', borderRadius: 10, background:'var(--bg-2)', boxShadow:'inset 0 0 0 1px var(--border)', overflow:'hidden' }}>
                {s.vis}
              </div>
  ...
  return (
    <div style={{ padding: 14, height:'100%', display:'flex', flexDirection:'column', justifyContent:'center', gap:8 }}>
      ...
      <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginTop:6 }}>
        {['6 mo','remote','no degree'].map(t => <span key={t} className="pill" style={{fontSize:10}}>{t}</span>)}
      </div>
    </div>
  );
};
      ...
      {[
        ...
        <div key={i} style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          <div style={{ flex:1, height:10, borderRadius:4, background:'var(--bg-1)', position:'relative', overflow:'hidden' }}>
            <div style={{ position:'absolute', left:r.x, width:r.w, top:0, bottom:0, background: `linear-gradient(90deg, ${r.c}, ${r.c}88)`, borderRadius:4,
              animation:`drawIn 1.2s ${i*0.15}s cubic-bezier(.16,1,.3,1) both`, transformOrigin:'left' }}/>
          </div>
        </div>
      ))}
      <style>{`@keyframes drawIn { from{ transform: scaleX(0);} to{ transform: scaleX(1);} }`}</style>
    </div>
  );
};
  ...
  <div style={{ position:'relative', height:'100%' }}>
    <div style={{position:'absolute', inset:14, borderRadius:8, background:'var(--bg-1)', boxShadow:'inset 0 0 0 1px var(--border)', overflow:'hidden'}}>
      <div className="code-block" style={{ padding: 10, fontSize:10, color:'var(--text-mid)' }}>
        <div><span className="tok-kw">import</span> torch</div>
        <div><span className="tok-kw">class</span> <span className="tok-fn">Model</span>(nn.Module):</div>
        <div style={{ paddingLeft: 10 }}><span className="tok-kw">def</span> <span className="tok-fn">forward</span>(self, x):</div>
        <div style={{ paddingLeft: 20 }}><span className="tok-kw">return</span> self.layers(x)</div>
      </div>
      <div style={{
...
const CredVis = () => (
  <div style={{ height:'100%', display:'flex', alignItems:'center', justifyContent:'center', perspective:'600px' }}>
    <div style={{
      ...
      display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:4, position:'relative', overflow:'hidden'
    }}>
    ...
    <style>{`@keyframes gentleFloat { 0%,100%{ transform: translateY(0) rotate(-2deg);} 50%{ transform: translateY(-6px) rotate(2deg);}}`}</style>
  </div>
);
...
const BentoEvaluation = () => {
  return (
    <section id="eval" style={{ padding:'120px 0', position:'relative' }}>
      <div className="container">
        <SectionHead eyebrow="The hero feature" title={<>Multimodal evaluation, in <i>four stages</i>.</>}
          sub="We don't just run your code — we look at the screens you ship. Gemma 4 12B's vision sees what users see."/>
        <div style={{
          marginTop: 56, display:'grid', gridTemplateColumns:'2fr 1fr 1fr', gridTemplateRows:'auto auto', gap: 16, gridAutoFlow:'dense'
            ...
            <div style={{ marginTop: 14, display:'grid', gap: 10 }}>
              {[
                ...
                <div key={l} style={{ display:'flex', alignItems:'center', gap:10 }}>
                  <div style={{ width:28, height:28, borderRadius:8, background:'var(--bg-2)', display:'flex', alignItems:'center', justifyContent:'center', color:'var(--text-mid)', boxShadow:'inset 0 0 0 1px var(--border)'}}><Icon name={ic} size={14}/></div>
                  <div style={{ flex:1 }}>
          ...
          <BentoTile>
            ...
            <div className="mono" style={{ fontSize:10, color:'var(--text-mid)', marginTop:4, wordBreak:'break-all' }}>sig: e3b0c4 ... 9b427</div>
          </BentoTile>

          <BentoTile span={{col:2, row:1}}>
            <SmallEyebrow>Provider fallback chain</SmallEyebrow>
            <p style={{ color:'var(--text-mid)', fontSize:14, marginTop: 10 }}>If a provider is rate-limited or down, we fail over without losing your run state.</p>
            <div style={{ marginTop: 16, display:'flex', flexWrap:'wrap', gap:12, alignItems:'center' }}>
              ...
              <span style={{ fontSize:12, color:'var(--text-mid)' }}>· latency p50 <span className="mono">1.4s</span> · success <span className="mono">99.7%</span></span>
            </div>
          </BentoTile>
        </div>
      </div>
...
const CountUp = ({ value, suffix }) => {
  ...
  lUseEffect(() => {
    ...
    const step = (t) => { if(!t0) t0 = t; const p = Math.min(1, (t-t0)/dur); const eased = 1 - Math.pow(1 - p, 3); setN(value * eased); if (p<1) raf = requestAnimationFrame(step); };
    raf = requestAnimationFrame(step);
    return () => cancelAnimationFrame(raf);
  }, [value]);
  return <div className="serif" style={{ fontSize: 22 }}>{Number.isInteger(value) ? Math.round(n) : n.toFixed(1)}<span style={{fontSize:12, color:'var(--text-mid)'}}>{suffix}</span></div>;
};
  ...
  <svg style={{ position:'absolute', inset:14, pointerEvents:'none' }} width="calc(100% - 28px)" height="calc(100% - 28px)" preserveAspectRatio="none" viewBox="0 0 200 140">
    {[
      ...
      <g key={i}>
        ...
        <text x={b.x+2} y={b.y - 1} fill={b.c} fontSize="4" fontFamily="JetBrains Mono">{b.l}</text>
      </g>
    ))}
  </svg>
);
...
const PortfolioPreview = ({ navigate }) => {
  const [tilt, setTilt] = lUseState({ x:0, y:0 });
  const ref = lUseRef(null);
  const onMove = (e) => {
  ...
  return (
    <section style={{ padding:'140px 0', position:'relative', overflow:'hidden' }}>
      <div className="container">
        <SectionHead eyebrow="Public portfolio" title={<>One link. <i>Real proof.</i></>}
        ...
        <div ref={ref} onMouseMove={onMove} onMouseLeave={()=>setTilt({x:0,y:0})}
             style={{ marginTop: 56, position:'relative', perspective:'1500px' }}>
          <div style={{
            background:'var(--bg-1)', borderRadius:18,
            boxShadow:'0 80px 200px -40px rgba(0,0,0,0.7), inset 0 0 0 1px var(--border-strong)',
            overflow:'hidden', transform:`rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)`, transition:'transform .15s ease',
            ...
            <div style={{ display:'flex', alignItems:'center', gap:6, padding:'10px 14px', borderBottom:'1px solid var(--border)' }}>
              ...
              <div className="mono" style={{ fontSize:12, color:'var(--text-mid)', marginLeft: 14, padding:'4px 10px', background:'var(--bg-2)', borderRadius:6 }}>cairn.dev/u/mira-k</div>
            ...
            <div style={{ padding: 40, display:'grid', gridTemplateColumns:'1fr 1.5fr', gap:36 }} className="preview-grid">
              <div>
                ...
                <div style={{ marginTop:18, display:'flex', gap:8 }}>
                  <MagneticButton variant="ghost" onClick={()=>navigate('example')}>See live <Icon name="arrow-up-right" size={12}/></MagneticButton>
                </div>
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap: 14 }}>
                {[
                  ...
                  <div key={p.t} className="card" style={{ padding: 14 }}>
                    <div style={{ height:60, borderRadius:8, background:'linear-gradient(135deg, '+(p.s>90?'rgba(52,211,153,0.3)':'rgba(99,102,241,0.3)')+', rgba(99,102,241,0.05))', marginBottom:10 }}/>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline' }}>
                      <div className="serif" style={{ fontSize:16 }}>{p.t}</div>
                      <div className="mono" style={{ fontSize:11, color: p.s>=90?'#6ee7b7':'#a5b4fc'}}>{p.s}/100</div>
                    </div>
...
const WhyGemma = () => {
  ...
  return (
    <section style={{ padding:'120px 0' }}>
      <div className="container">
        <SectionHead eyebrow="Intentional model selection" title={<>Why <i>Gemma 4</i>, three sizes deep.</>}
          sub="Different problems want different brains. We route each task to the smallest model that handles it well."/>
        <div style={{ marginTop: 56, display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap: 18 }} className="why-grid">
          {cards.map((c, i) => (
            <div key={c.sz} className="card" style={{ padding: 28, position:'relative', overflow:'hidden' }} data-magnet>
              <div style={{ position:'absolute', top:-40, right:-40, width:200, height:200, borderRadius:999, background:`radial-gradient(circle, ${c.tone}33, transparent 60%)` }}/>
...
const Pricing = ({ navigate }) => {
  const tiers = [
    { name:'Free', price:'$0', cadence:'forever', desc:'Everything to ship one verified credential.', cta:'Start free', feats:['1 active path','3 project evaluations / mo','Public portfolio','Gemma 4 routing','Basic support'] },
    { name:'Pro', price:'$19', cadence:'/ month', desc:'For serious career switchers.', cta:'Go Pro', feats:['Unlimited paths','Unlimited evaluations','Re-eval & coach feedback','Recruiter direct outreach','Custom domain on portfolio','Priority queue'], featured:true },
    { name:'Teams', price:'$9', cadence:'/ seat / mo', desc:'For bootcamps & talent teams.', cta:'Book a call', feats:['Cohort dashboard','Org branding','SSO + SCIM','Bulk verification API','Recruiter analytics','Dedicated CSM'] },
  ];
  return (
    <section id="pricing" style={{ padding:'120px 0' }}>
      <div className="container">
        <SectionHead eyebrow="Pricing" title={<>Free to start. <i>Proof</i> isn't a paywall.</>}/>
        <div style={{ marginTop: 56, display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap: 16 }} className="price-grid">
          {tiers.map(t => {
            const inner = (
              <div style={{ padding: 32, height:'100%', display:'flex', flexDirection:'column' }}>
                ...
                <div style={{ marginTop: 24 }}>
                  <MagneticButton variant={t.featured?'primary':'ghost'} onClick={()=>navigate('onboarding')}>
            ...
            return t.featured ? (
              <div key={t.name} className="aurora-border" style={{ borderRadius: 20 }}>
                <div className="card" style={{ background:'var(--bg-1)', borderRadius: 19, boxShadow:'none' }}>{inner}</div>
              </div>
            ) : (
...
const FAQ = () => {
  ...
  return (
    <section style={{ padding:'120px 0', borderTop:'1px solid var(--border)'}}>
      <div className="container" style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:80 }} className="faq-grid">
        <div>
          <SmallEyebrow>FAQ</SmallEyebrow>
          <h2 className="serif" style={{ fontSize: 56, margin:'12px 0 0', lineHeight: 0.95, letterSpacing:'-.03em' }}>The <i>honest</i> answers.</h2>
          <p style={{ color:'var(--text-mid)', marginTop: 18, fontSize:15, lineHeight: 1.6 }}>Still curious? Email <a href="#" style={{color:'#a5b4fc'}}>hi@cairn.dev</a> — we read everything.</p>
        </div>
        <div>
          {faqs.map((f, i) => (
            <div key={i} style={{ borderTop:'1px solid var(--border)', padding:'20px 0' }}>
              <button onClick={()=>setOpen(open===i?-1:i)} style={{ width:'100%', background:'none', border:'none', display:'flex', alignItems:'center', justifyContent:'space-between', padding:0, color:'var(--text-hi)', textAlign:'left' }}>
                <span className="serif" style={{ fontSize: 22 }}>{f.q}</span>
                <span style={{ width:32, height:32, borderRadius:999, display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg-2)', boxShadow:'inset 0 0 0 1px var(--border)', transform: open===i?'rotate(45deg)':'rotate(0)', transition:'transform .35s cubic-bezier(.16,1,.3,1)' }}>
...
const SectionHead = ({ eyebrow, title, sub, align='left' }) => (
  <div style={{ textAlign: align, maxWidth: 760, margin: align==='center'?'0 auto':'0' }}>
    ...
    <h2 className="serif" style={{ fontSize:'clamp(36px, 5.2vw, 64px)', margin:0, lineHeight:0.98, letterSpacing:'-.03em', textWrap:'balance' }}>{title}</h2>
    {sub && <p style={{ color:'var(--text-mid)', fontSize:17, marginTop: 18, maxWidth: 600, lineHeight:1.6 }}>{sub}</p>}
  </div>
);
